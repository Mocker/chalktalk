...
// GET AND SET STATE DATA VIA THE PERSISTENT SERVER.
   ...
   function Server() {
      ...
      this.connectSocket = function(localHost) {
         this._localhost = localHost;
         this.socket = new WebSocket("ws://" + localHost + ":22346");
...
// COLOR UTILITIES.
   ...
   function rgbToColor(rgb) {
      return 'rgb(' + floor(rgb[0]) + ','
                    + floor(rgb[1]) + ','
                    + floor(rgb[2]) + ')';
   }

   function rgbaToColor(rgba) {
      return 'rgba(' + floor(rgba[0]) + ','
                     + floor(rgba[1]) + ','
                     + floor(rgba[2]) + ','
                     + floor(rgba[3]) + ')';
   }
...
// MATH CONSTANTS AND FUNCTIONS
   ...
   function MinHeap() {
      var H = [Number.NEGATIVE_INFINITY], I = [], size = 0;
      this.insert = function(x, item) {
         var i = ++size; 
         for ( ; H[Math.floor(i/2)] > x ; i >>= 1) {
            H[i] = H[i >> 1];
            I[i] = I[i >> 1];
         }
         H[i] = x;
      ...
      this.minValue = function() { return H[1]; }
      this.deleteMin = function() {
         var minItem = I[1], maxItem = I[size], max = H[size--], i = 1;
         for (let c = 1 ; 2*i < size ; i = c) {
            if ((c = 2*i) != size && H[c+1] < H[c])
               c++;
            if (max > H[c]) {
               H[i] = H[c];
               I[i] = I[c];
   ...
   function mix3(a, b, c, t) {
      return t < .5 ? mix(a, b, 2 * t) : mix(b, c, 2 * t - 1);
   }
   ...
   function orthonormalBasis(axis, x,y,z) {
      switch (axis) {
      case 0:
         x = normalize(x);
         y = normalize(y);
         ...
         let u = abs(dot(x, y)),
             v = abs(dot(x, z));
         if (u == 0 && v == 0)
	    return [ x, y, normalize(cross(x, y)) ];
         let y1 = cross(cross(x, y), x),
             z1 = cross(x, cross(z, x)),
             y2 = mix(y1, cross(z1, x), v / (u + v)),
             z2 = mix(cross(x, y1), z1, v / (u + v));
         return [ x, normalize(y2), normalize(z2) ];
      case 1:
         var b = orthonormalBasis(0, y,z,x);
         return [ b[2], b[0], b[1] ];
      case 2:
         var b = orthonormalBasis(0, z,x,y);
         return [ b[1], b[2], b[0] ];
      }
   }
   function pca(xyw) {
...
// 3D GEOMETRY UTILITIES.
   ...
   function computeNormals(V, T) {
	 ...
	 for (let i = 0 ; i < 3 ; i++) {
	    if (! isNaN(uu)) N[a + i] += u[i] / uu;
	    if (! isNaN(vv)) N[b + i] += v[i] / vv;
	    if (! isNaN(ww)) N[c + i] += w[i] / ww;
         }
      }
