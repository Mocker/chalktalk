...
if(typeof('require') != 'undefined' && typeof(ipcRenderer) == "undefined" ) {
   ...
   function initEventHandlers(canvas) {
	 ...
	 return keyCode;
      ...
      canvas.oncontextmenu = function(event) {
         setTextMode(false);
         console.log("right click -- pop up radial menu");
         let menuItems = [ //TODO:: sketchPage holds available interactions for a selected sketch. The following is sample menu
            {
               id   : 'walk',
               title: 'Walk',
               icon: '#walk'
         ...
         //TODO:: we should create the menu once and just update when the options change
         if(window.global.svgMenu) window.global.svgMenu.close();
         document.querySelectorAll('div.menuHolder').forEach((o)=>o.remove());
         const holderSize = 400;
         window.global.svgMenu = new RadialMenu({
            parent      : document.getElementById('root_div'), //have to place it under here
            size        : holderSize,
            closeOnClick: true,
            menuItems   : menuItems,
         ...
         document.querySelector('div.menuHolder').style.top=(event.clientY-(holderSize/2))+"px";
         document.querySelector('div.menuHolder').style.left=(event.clientX-(holderSize/2))+"px";
         window.global.svgMenu.open();
         return false;
      };
...
*/
   ...
   function _g_text(str, point, matrix) { displayStrokes.text(str, point, matrix); }
...
// TODO: FIGURE OUT HOW TO LOAD THE NEWLY CREATED SKETCH TYPE INTO CURRENT GLYPHS.
   ...
   function loadSketchLibs(callback) {
      try {
         ...
         //TODO:: all these requests should be moved to their own class
         if( ipcRenderer ) { //electron client, can call the main process directly
            console.log("INVOKE ls_sketchlibs");
            ipcRenderer.invoke('ls_sketchlibs').then((result) => { console.log("RESULT",result);
               var ls = result.trim().split("\n"); console.log(ls);
   ...
   function loadSketches() {
      try {
         if( ipcRenderer ) { //electron client, can call the main process directly
            ipcRenderer.invoke('ls_sketches').then((result) => {
               var ls = result.trim().split("\n");
               var remainingRequests = {count: ls.length};
               for (var n = 0; n < ls.length; n++) {
                  var filename = ls[n];
   ...
   function importSketch(filename, remainingRequests) {
      if( ipcRenderer ) { //electron client, can call the main process directly
         console.log("Invoke sketched");
         ipcRenderer.invoke('sketches', filename).then((result) => {
            var text = result;
            ...
            // OTHERWISE LOAD THE NEW SKETCH TYPE.
               ...
               sketchFileData[typeName] = [ filename, text ];
            }
   ...
   function ChalkTalkStart(rootSelector) {
...
//    + " <input id='soundfileinput' type='file' style='visibility:hidden' /> </input>"
      ;
      window.bodyElement = document.querySelector(rootSelector);
      ...
      socket = server.connectSocket(localHost);
...
var sketchDragY = 0;
   ...
   function assignCollection() {
      ...
      if( ipcRenderer ) {
         let result = ipcRenderer.invoke('set_collections', collections);
      } else { //ajax server set
         server.set('state/collections', collections);
      }
   }

   function unassignCollection() { console.log("unassign collection");
...
*/
      ...
      if (window.forceSetPageAtTime !== undefined && forceSetPageAtTime < time) {
         ...
         if( ipcRenderer ) { //electron client, can call the main process directly
            console.log("INVOKE ls_state");
            ipcRenderer.invoke('ls_state').then((result) => {  console.log("collections", result);
               if (result.indexOf('collections') >= 0) {
                  // THEN LOAD THE COLLECTIONS INDEX.
                  ipcRenderer.invoke('state/collections', function(value) { console.log("loaded collections", value);
                     collections = value;
      ...
      if (isDef(window[g.name].animate)) {
        ...
        // beyond reasonable doubt not to be buggy
         ...
         if (isShowingTime) {
            ...
            _g.font = '12pt Arial';
            _g_text(timeString, [w - textWidth(timeString) - 6 - _g.panX,
                                 h - 6 - _g.panY,
                                 -6 - _g.panZ]);
            annotateEnd();
         }
...
////////////////////////////////////////////////////////////
   ...
   function setPage(index) {
	 ...
	 if (sketchTypes === undefined)
      ...
      window.global = renderer.scene.root;
   }
